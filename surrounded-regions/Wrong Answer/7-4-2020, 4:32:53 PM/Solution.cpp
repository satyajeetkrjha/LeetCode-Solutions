...
public:
    bool boundary_check(vector<vector<char>>& board,int i ,int j){
        ...
        if(  i ==0  || j==0 || i ==rows-1 || j ==columns-1  || board[i][j]=='0')
        return false;
        return true;
    }
    bool boundary_check_connection(vector<vector<char>>& board,int i ,int j){
      if (board[i][j]=='O')
          return false;
        return true;
    }
    ...
    void dfs (vector <vector <char>>&board,int i ,int j,int rows,int columns){
        ...
        for(int index=0;index<4;index++){
           ...
           // cout<<"ndx and ndy are"<<ndx<<" "<<ndy<<endl;
             if(boundary_check(board,i,j)==true && ndx >=0 && ndx<=rows-1 && ndy >=0 && ndy<=columns-1 && boundary_check_connection(board,ndx,ndy) ==true){
                 board[i][j]='X';
